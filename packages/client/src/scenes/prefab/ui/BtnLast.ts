...
export default class BtnLast extends Phaser.GameObjects.Container {

	constructor(scene: Phaser.Scene, x?: number, y?: number) {
		super(scene, x ?? 0, y ?? 0);

		// btnMainA
		const btnMainA = scene.add.image(0, 0, "Backward_BTNA");
		btnMainA.setOrigin(0, 0);
		this.add(btnMainA);

		// btnMain
		const btnMain = scene.add.image(0, 0, "Backward_BTN");
