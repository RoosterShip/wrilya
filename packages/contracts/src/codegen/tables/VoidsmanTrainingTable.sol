...
library VoidsmanTrainingTable {
   ...
   */
  function decodeStatic(bytes memory _blob) internal pure returns (uint256 time, FieldEnum field) {
    time = (uint256(Bytes.getBytes32(_blob, 0)));

    field = FieldEnum(uint8(Bytes.getBytes1(_blob, 32)));
  }
  ...
  function decode(
    ...
    EncodedLengths,
    bytes memory
  ) internal pure returns (uint256 time, FieldEnum field) {
   ...
   */
  function encodeStatic(uint256 time, FieldEnum field) internal pure returns (bytes memory) {
   ...
   */
  function encode(uint256 time, FieldEnum field) internal pure returns (bytes memory, EncodedLengths, bytes memory) {
    bytes memory _staticData = encodeStatic(time, field);
