...
library CurrencyTable {
   ...
   */
  function decodeStatic(bytes memory _blob) internal pure returns (uint256 tokens, uint256 credits, uint256 debit) {
  ...
  function decode(
    ...
    EncodedLengths,
    bytes memory
  ) internal pure returns (uint256 tokens, uint256 credits, uint256 debit) {
    (tokens, credits, debit) = decodeStatic(_staticData);
  }
   ...
   */
  function encodeStatic(uint256 tokens, uint256 credits, uint256 debit) internal pure returns (bytes memory) {
    return abi.encodePacked(tokens, credits, debit);
  }
